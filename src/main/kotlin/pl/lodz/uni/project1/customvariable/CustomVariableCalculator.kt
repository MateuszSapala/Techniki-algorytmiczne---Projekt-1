...
class CustomVariableCalculator {
    companion object {
        ...
        private fun floatSum(valA: CustomVariableDigit?, valB: CustomVariableDigit?): Pair<CustomVariableDigit?, Byte> {
            ...
            return Pair(result?.reverse(), carried)
        }
        ...
        fun subtract(valA: CustomVariable, valB: CustomVariable): CustomVariable {
            ...
            if (!sameSign) {
                return add(
                    valA,
                    CustomVariable(CustomVariableDigit(valA.sign(), valB.getInt()), valB.getFloat())
                )
            }
            ...
            val sign = if (change) (a.sign() * -1).toByte() else a.sign()
            return CustomVariable(
                CustomVariableDigit(sign, int),
                floatResult.first
            )
        }
        ...
        private fun floatSubtraction(
            ...
            return Pair(result?.reverse(), lent)
        }
